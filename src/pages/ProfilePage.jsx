...
export default function ProfilePage({
  user = { avatarUrl: '', name: 'Ravi Kumar', email: 'ravi@example.com', phone: '' },
  onUpdateProfile, onChangePassword,
  onEnable2FA, onVerify2FA, onDisable2FA, onDownloadBackupCodes,
  onRevokeSessions, onToggleProvider, onSaveNotifications,
  ...
  const [notifs, setNotifs] = useState({ emailProduct: true, emailSecurity: true, emailReminders: false, pushAnnouncements: false });
  ...
  const handleSaveProfile = async () => {
    try { setSavingProfile(true); await onUpdateProfile?.(profile); alert('Profile updated'); }
    catch { alert('Failed to update profile'); } finally { setSavingProfile(false); }
  };

  const handleChangePassword = async () => {
    if (!pwd.next || pwd.next !== pwd.confirm) { alert('New passwords do not match'); return; }
    try { setSavingPwd(true); await onChangePassword?.({ currentPassword: pwd.current, newPassword: pwd.next }); setPwd({ current: '', next: '', confirm: '' }); alert('Password updated'); }
    catch { alert('Failed to change password'); } finally { setSavingPwd(false); }
  };

  const handleEnable2FA = async () => {
    try { setSaving2FA(true); const setup = await onEnable2FA?.(); setTwoFASetup(setup || {}); }
    catch { alert('Failed to start 2FA setup'); } finally { setSaving2FA(false); }
  };
  const handleVerify2FA = async () => {
    if (!twoFACode) return;
    try { setSaving2FA(true); await onVerify2FA?.(twoFACode); setTwoFAEnabled(true); setTwoFACode(''); alert('2FA enabled'); }
    catch { alert('Invalid verification code'); } finally { setSaving2FA(false); }
  };
  const handleDisable2FA = async () => {
    try { setSaving2FA(true); await onDisable2FA?.(); setTwoFAEnabled(false); setTwoFASetup({ otpauthUrl: '', qrSvg: '' }); alert('2FA disabled'); }
    catch { alert('Failed to disable 2FA'); } finally { setSaving2FA(false); }
  };
  const handleDownloadBackupCodes = async () => {
    try { const res = await onDownloadBackupCodes?.(); if (!res) return;
    ...
    } catch { alert('Failed to download backup codes'); }
  };
  const handleRevokeSessions = async () => {
    try { setRevoking(true); await onRevokeSessions?.(); alert('Other sessions revoked'); }
    catch { alert('Failed to revoke sessions'); } finally { setRevoking(false); }
  };
  const handleToggleProvider = async (provider, enabled) => {
    try { setSavingProviders(true); await onToggleProvider?.(provider, enabled); setProviders((p) => ({ ...p, [provider]: enabled })); }
    catch { alert('Failed to update provider'); } finally { setSavingProviders(false); }
  };
  const handleSaveNotifs = async () => {
    try { setSavingNotifs(true); await onSaveNotifications?.(notifs); alert('Notification preferences saved'); }
    catch { alert('Failed to save preferences'); } finally { setSavingNotifs(false); }
  };
  const handleDeactivate = async () => {
    if (!deactivateReason.trim()) { alert('Please share a short reason to help improve the service'); return; }
    if (!confirm('Temporarily deactivate this account? You can restore it later.')) return;
    try { await onDeactivateAccount?.({ reason: deactivateReason.trim() }); alert('Account deactivated — sign in later to reactivate'); }
    catch { alert('Failed to deactivate'); }
  };
  const handleDelete = async () => {
    if (deleteConfirm.text !== 'DELETE') { alert('Type DELETE to confirm'); return; }
    if (!deleteConfirm.password) { alert('Enter password to confirm'); return; }
    if (!confirm('This permanently deletes the account and all data. Continue?')) return;
    try { setDeleting(true); await onDeleteAccount?.({ confirmText: deleteConfirm.text, password: deleteConfirm.password }); alert('Account deleted'); window.location.href = '/'; }
    catch { alert('Failed to delete account'); } finally { setDeleting(false); }
  };
...
/>
          ...
          <div className="bg-white rounded-xl shadow p-4">
            <h2 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2"><Shield className="w-4 h-4" /> Security</h2>
            <div className="space-y-2">
        ...
        <section className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-xl shadow p-4">
            <h2 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2"><Bell className="w-4 h-4" /> Notifications</h2>
          ...
          <div className="bg-white rounded-xl shadow p-4">
            <h2 className="text-sm font-semibold text-red-700 mb-3 flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> Danger Zone</h2>
            <div className="space-y-3">
              <div>
                ...
                <input className="w-full p-2 border rounded text-sm" placeholder="Reason (optional but helpful)" value={deactivateReason} onChange={(e) => setDeactivateReason(e.target.value)} />
                <button onClick={handleDeactivate} className="mt-2 px-3 py-2 bg-yellow-50 text-yellow-800 rounded hover:bg-yellow-100">Deactivate account</button>
              </div>
